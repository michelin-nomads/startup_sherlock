    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold">
                  {analysisData.analysis.overallScore}/100
                </span>
              </div>
              <Progress
                value={analysisData.analysis.overallScore}
                className="h-2"
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="flex items-center justify-between pt-2 border-t">
            <div className="flex items-center gap-2">
              ...
              >
                {Math.abs(
                  analysisData.analysis.overallScore - marketTrends.overallScore
                ).toFixed(1)}{" "}
                points difference
              </span>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            {Object.entries(analysisData?.analysis?.metrics || {}).map(
              ([metric, companyValue]) => {
                const marketValue =
                  marketTrends.metrics[
                    metric as keyof typeof marketTrends.metrics
                  ];
                return (
                  <div key={metric} className="space-y-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      >
                        {companyValue > marketValue ? "+" : ""}
                        {(companyValue - marketValue).toFixed(1)} points
                      </span>
                    </div>
                  </div>
                );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold">
                  {formatCurrency(
                    analysisData.analysis.recommendation?.targetInvestment || 0
                  )}
                </span>
              </div>
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold">
                  {formatCurrency(
                    marketTrends.recommendation?.targetInvestment || 0
                  )}
                </span>
              </div>
              <div className="flex items-center gap-2 pt-2 border-t">
                {getComparisonIcon(
                  analysisData.analysis.recommendation?.targetInvestment,
                  marketTrends.recommendation?.targetInvestment
                )}
                <span
                  className={`text-sm ${getComparisonColor(
                    analysisData.analysis.recommendation?.targetInvestment,
                    marketTrends.recommendation?.targetInvestment
                  )}`}
                >
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold">
                  {analysisData.analysis.recommendation?.expectedReturn}x
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Market Average</span>
                <span className="font-bold">
                  {marketTrends.recommendation?.expectedReturn || 0}x
                </span>
              </div>
              <div className="flex items-center gap-2 pt-2 border-t">
                {getComparisonIcon(
                  analysisData.analysis.recommendation?.expectedReturn,
                  marketTrends.recommendation?.expectedReturn
                )}
                <span
                  className={`text-sm ${getComparisonColor(
                    analysisData.analysis.recommendation?.expectedReturn,
        ...
        <div className="pt-4 border-t">
          ...
          <p className="text-sm text-muted-foreground">
            This analysis shows how {analysisData.startup.name} compares to
            current market trends. Green indicators show areas where the company
            performs better than market average, while red indicators highlight
